...
- **图片上传：** 校验登录、文件实际大小、文件签名与图片尺寸，限制最大宽高和总像素数；接受 JPEG、PNG、WebP 与 GIF（含动画：原始字节流直存 R2，浏览器原生播放），拒绝 SVG / HTML 等主动内容。对象键由服务端生成并绑定上传者，发布时验证图片归属；返回正确的 `Content-Type` 与 `X-Content-Type-Options: nosniff`。
