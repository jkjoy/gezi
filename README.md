...
| 管理员 | 调整参数、搜索用户、批量发积分、核实捐助到账、触发日结、查看操作记录 |
...
- 内容块文字按区域短边等比例缩放，窗口尺寸变化时重算；悬停内容块显示完整信息（全文、位置尺寸、链接），点击带链接的内容块直接在新窗口打开（`noopener noreferrer`）。移动端允许缩放或滚动查看墙面，图片在所选矩形中按比例裁切；悬停浮层为桌面交互，触摸端的等价展示列为后续迭代。
...
管理员不采用“首个公开注册用户自动成为管理员”的规则。首位管理员通过受控的一次性初始化流程绑定指定账户，初始化后关闭入口；普通注册不能申请管理员角色。
